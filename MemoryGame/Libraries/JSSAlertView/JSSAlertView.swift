...
open class JSSAlertView: UIViewController {
	...
	open override func viewDidLayoutSubviews() {
		...
		// position the title
        var titleAttr: Dictionary<NSAttributedString.Key, Any>? = nil
        if self.titleLabel != nil {
            let titleString = titleLabel.text! as NSString
            let titleSize = CGSize(width: contentWidth, height: 90)
            titleAttr = [NSAttributedString.Key.font: titleLabel.font!]
		...
		if !noButtons {
			...
			// set button fonts
			if buttonLabel != nil {
				buttonLabel.font = UIFont(name: buttonFont, size: 27)
			}
			if cancelButtonLabel != nil {
				cancelButtonLabel.font = UIFont(name: buttonFont, size: 27)
			}
			yPos += buttonHeight
		}else{
			yPos += padding
	...
	public func show(_ viewController: UIViewController,
        ...
        view.backgroundColor = UIColorFromHex(0x000000, alpha: 0.2)//DEREK
		...
		alertBackgroundView.layer.cornerRadius = 12
        ...
        if let title = title {
            ...
            titleLabel.font = UIFont(name: self.titleFont, size: 28)
...
extension JSSAlertView {
	...
	func setFont(_ fontStr: String, type: FontType) {
		...
		case .button:
			...
			if let font = UIFont(name: self.buttonFont, size: 27) {
